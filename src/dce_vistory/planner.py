...
def _safe_make(cls, kwargs: Dict[str, Any]):
    ...
    if names:
        obj = cls(**{k: v for k, v in kwargs.items() if k in names})
...
def _ending_synonym(target: str) -> str:
    ...
    return {'happy':'joy','happiness':'joy','sad':'sadness','angry':'anger','fearful':'fear'}.get(target, target or 'resolution')

class DCEPlanner:
    """Schema-compatible DCEE planner.

    The code keeps the existing class name DCEPlanner/DCEPlan for compatibility,
    but the generated plan is DCEE: Desire-Conflict-Event-Ending Emotion.
    """
    ...
    def build_seed(self, sample: Dict[str, Any], image_summary: ImageUnderstanding | None) -> StorySeed:
        text = self.llm.generate(SYSTEM_NARRATIVE, story_seed_prompt(sample, _to_dict(image_summary) if image_summary else None), temperature=self.temperature, max_tokens=self.max_tokens)
        ...
        seed = _safe_make(StorySeed, {
            'image_summary': image_summary, 'text_prompt': sample.get('text_prompt',''), 'protagonist': sample.get('protagonist',''),
        ...
        for row in data.get('character_profiles', []):
            if isinstance(row, dict):
                profiles.append(_safe_make(CharacterProfile, {
                    ...
                    'signature_items': row.get('signature_items', []), 'color_palette': row.get('color_palette',''), 'identity_anchor_prompt': row.get('identity_anchor_prompt',''),
                }))
        protagonist = sample.get('protagonist','protagonist')
        if not any(getattr(p,'name','').lower()==protagonist.lower() or getattr(p,'role','')=='protagonist' for p in profiles):
            ...
            profiles.insert(0, _safe_make(CharacterProfile, {
                ...
                'face': f'recognizable consistent face or character features based on: {image_hint}', 'hair': 'same hairstyle or head shape in every frame',
    ...
    def generate_abstract(self, seed: StorySeed) -> str:
        return self.llm.generate(SYSTEM_NARRATIVE, story_abstract_prompt(_to_dict(seed)), temperature=self.temperature, max_tokens=900).strip()

    def generate_dce_plan(self, seed: StorySeed, abstract: str) -> DCEPlan:
        data = extract_json(self.llm.generate(SYSTEM_NARRATIVE, dce_plan_prompt(_to_dict(seed), abstract), temperature=self.temperature, max_tokens=self.max_tokens))
        plan = _safe_make(DCEPlan, {
            'protagonist': data.get('protagonist', getattr(seed,'protagonist','')), 'desire': data.get('desire',''), 'fear': data.get('fear',''),
            'misbelief': data.get('misbelief',''), 'obstacle': data.get('obstacle',''), 'conflict': data.get('conflict',''),
            'event_spine': data.get('event_spine', data.get('event_chain', [])), 'turning_point': data.get('turning_point',''),
            'target_ending_emotion': data.get('target_ending_emotion', getattr(seed,'target_ending_emotion','')), 'ending_state': data.get('ending_state',''),
            'moral_or_theme': data.get('moral_or_theme',''), 'planning_type': 'DCEE',
        })
        plan.planning_type = 'DCEE'
        plan.event_chain = getattr(plan, 'event_spine', data.get('event_chain', []))
        return plan

    def generate_emotion_arc(self, seed: StorySeed, abstract: str, dce_plan: DCEPlan, num_frames: int) -> EmotionArc:
        data = extract_json(self.llm.generate(SYSTEM_NARRATIVE, emotion_arc_prompt(_to_dict(seed), abstract, _to_dict(dce_plan), num_frames), temperature=self.temperature, max_tokens=self.max_tokens))
        ...
        return _safe_make(EmotionArc, {'states': states, 'intensities': [max(1,min(5,int(x))) for x in intensities], 'rationale': data.get('rationale','')})

    def generate_storyboard(self, seed: StorySeed, abstract: str, dce_plan: DCEPlan, emotion_arc: EmotionArc) -> List[StoryboardFrame]:
        states, intensities = getattr(emotion_arc,'states',[]), getattr(emotion_arc,'intensities',[])
        num_frames = len(states)
        rows = extract_json(self.llm.generate(SYSTEM_NARRATIVE, storyboard_prompt(_to_dict(seed), abstract, _to_dict(dce_plan), _to_dict(emotion_arc), num_frames), temperature=self.temperature, max_tokens=self.max_tokens))
        if isinstance(rows, dict): rows = rows.get('storyboard', rows.get('frames', rows))
        if not isinstance(rows, list): raise ValueError(f'Storyboard must be a list, got {type(rows)}: {rows}')
        ...
        world_context = getattr(seed,'world_context',{}) or {}; image_summary = getattr(seed,'image_summary',None)
        ...
        for idx, row in enumerate(rows):
            ...
            event = row.get('event', event_chain[min(idx, len(event_chain)-1)] if event_chain else '')
            event_causal_role = row.get('event_causal_role', f'This event causes or intensifies {emotion} and moves the story toward the ending emotion.')
            ...
            if not isinstance(env, list): env = [str(env)]
            env += [f"lighting style: {rule['lighting']}", f"composition supports {emotion}: {rule['composition']}", 'full-color environment, not grayscale']
            ...
            shot_type, camera_distance = choose_shot_type(idx, len(rows), narrative_function), choose_camera_distance(choose_shot_type(idx, len(rows), narrative_function))
            key_objects = row.get('key_objects', [])
            if not isinstance(key_objects, list): key_objects = [str(key_objects)]
            evidence = ([event] if event else []) + key_objects[:3]
            if row.get('desire_link',''): evidence.append(f"evidence of desire: {row.get('desire_link')}")
            evidence.append(f"event causal role: {event_causal_role}")
            must_show = key_objects[:3] + [f'visible event: {event}', f'facial evidence of {emotion}', f'body evidence of {emotion}', f'visual cause of {emotion}', 'full-color emotional lighting']
            ...
            frame = _safe_make(StoryboardFrame, {
                'frame_id': int(row.get('frame_id', idx+1)), 'caption': row.get('caption',''), 'narrative_function': narrative_function,
                'event': event, 'event_causal_role': event_causal_role, 'protagonist_state': row.get('protagonist_state',''), 'desire_link': row.get('desire_link',''),
                'conflict_level': int(row.get('conflict_level',1)), 'emotion': emotion, 'emotion_intensity': intensity, 'visual_focus': row.get('visual_focus',''),
                ...
                'cinematic_cue': row.get('cinematic_cue', f'{shot_type}, cinematic storytelling composition'), 'scene_location': scene_location,
                'time_of_day': time_of_day, 'weather': weather, 'atmosphere': row.get('atmosphere','') or f'{emotion} atmosphere',
                ...
                'composition_rule': f'{shot_type}, {camera_distance} distance, {rule["composition"]}. The scene must show the event and the visual cause of the protagonist emotion.',
                'quality_rule': QUALITY_SUFFIX, 'negative_prompt': neg, 'shot_type': shot_type, 'camera_distance': camera_distance,
                'color_palette': rule['palette'], 'lighting_style': rule['lighting'], 'must_show': must_show, 'emotion_evidence': evidence,
            })
            frames.append(frame); prev_emotion = emotion; prev_world = {'scene_location': scene_location, 'weather': weather}
        return frames
