...
def _extract_full_story_sentences(data: Any) -> Dict[str, Any]:
    if isinstance(data, dict):
        title = str(data.get("story_title", "")).strip()
        rows = data.get("sentences") or data.get("story_sentences") or data.get("frames") or data.get("story") or []
        ...
        if isinstance(rows, list):
            ...
            rows = []
        return {"story_title": title, "sentences": rows}
    if isinstance(data, list):
        rows = [r for r in data if isinstance(r, dict)]
        return {"story_title": "", "sentences": rows}
...
""".strip()
    ...
    def generate_abstract(self, seed: StorySeed) -> str:
        ...
        return text
    def generate_full_story(self, seed: StorySeed, abstract: str, dce_plan: DCEPlan, emotion_arc: EmotionArc, num_frames: int) -> Dict[str, Any]:
        prompt = (
            full_story_prompt(_to_dict(seed), abstract, _to_dict(dce_plan), _to_dict(emotion_arc), num_frames)
            + "\n\nSTRICT FULL STORY REQUIREMENTS:\n"
            + f"- Return exactly {num_frames} sentences in JSON.\n"
            + "- Each sentence must map to one frame and one moment in time.\n"
            + "- The final sentence must match the target ending emotion.\n"
            + "- Use concrete, visual language.\n"
        )
        data = self._llm_json_strict(
            prompt,
            stage="generate_full_story",
            max_tokens=1800,
            temperature=self.temperature,
            repair_hint="Return {'story_title': str, 'sentences': [{'frame_id':1,'sentence':'...'}]} with exact frame count.",
        )
        parsed = _extract_full_story_sentences(data)
        rows = parsed.get("sentences", [])
        if len(rows) != num_frames or _contains_generic_text(rows):
            repair_prompt = prompt + (
                "\n\nREPAIR: The previous response was invalid. "
                + f"Return EXACTLY {num_frames} sentence items with frame_id 1..{num_frames}."
            )
            data = self._llm_json_strict(
                repair_prompt,
                stage="repair_full_story",
                max_tokens=1800,
                temperature=0.0,
                repair_hint=f"Return exact JSON with {num_frames} sentence items.",
            )
            parsed = _extract_full_story_sentences(data)
            rows = parsed.get("sentences", [])

        if len(rows) != num_frames:
            raise RuntimeError(f"Full story sentence count mismatch. got={len(rows)}, expected={num_frames}")
        if _contains_generic_text(rows):
            raise RuntimeError("Full story contains generic placeholder text.")

        normalized = []
        states = list(getattr(emotion_arc, "states", []) or [])
        for idx, row in enumerate(rows):
            sentence = _clean_text(row.get("sentence") or row.get("text") or row.get("caption"))
            if not sentence:
                raise RuntimeError(f"Full story sentence {idx+1} is empty.")
            normalized.append({
                "frame_id": int(row.get("frame_id", idx + 1)),
                "sentence": sentence,
                "dcee_stage": _clean_text(row.get("dcee_stage") or row.get("stage") or ("Ending Emotion" if idx == num_frames - 1 else "Event")),
                "event": _clean_text(row.get("event") or sentence),
...
""".strip()
    ...
    def generate_storyboard(self, seed: StorySeed, abstract: str, dce_plan: DCEPlan, emotion_arc: EmotionArc, full_story: Dict[str, Any] | None = None) -> List[StoryboardFrame]:
        states = getattr(emotion_arc, "states", [])
        num_frames = len(states)
        story_rows = (full_story or {}).get("sentences", []) if isinstance(full_story, dict) else []
        story_alignment_block = ""
        if story_rows:
            story_alignment_block = "\n\nFULL STORY SENTENCE ALIGNMENT (each frame must visualize the matching sentence):\n" + json.dumps(story_rows, ensure_ascii=False, indent=2)
        prompt = (
    ...
    def _postprocess_storyboard(self, rows, seed, dce_plan, emotion_arc, full_story=None):
        ...
        for idx, row in enumerate(rows):
            ...
            if not isinstance(linked, dict):
                linked = {}
            story_row = story_rows[idx] if idx < len(story_rows) and isinstance(story_rows[idx], dict) else {}

            ev = row.get("event") or story_row.get("event") or linked.get("event")
            ...
            frame = _safe_make(
                ...
                {
                    "frame_id": int(row.get("frame_id", idx + 1)),
                    "caption": row.get("caption") or story_row.get("sentence") or evground,
                    ...
                    "dcee_stage": row.get("dcee_stage") or story_row.get("dcee_stage") or ("Ending Emotion" if idx == len(rows) - 1 else "Event"),
                    "story_sentence": story_row.get("sentence") or row.get("story_sentence") or row.get("caption") or evground,
                    "story_alignment_reason": story_row.get("alignment_reason") or row.get("story_alignment_reason") or f"This frame visualizes story sentence {idx + 1}.",
                    "story_title": (full_story or {}).get("story_title", "") if isinstance(full_story, dict) else "",
