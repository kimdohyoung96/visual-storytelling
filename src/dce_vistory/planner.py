...
def _safe_make(cls, kwargs: Dict[str, Any]):
    """
    schema.py 버전 차이 방지용 생성기.
    dataclass __init__에 없는 필드는 제거한 뒤 객체 생성 후 setattr로 다시 붙인다.
    """
            ...
            profiles.append(
                _safe_make(
                    CharacterProfile,
                    {
                        ...
                        "signature_items": row.get("signature_items", []),
    ...
    def generate_dce_plan(self, seed: StorySeed, abstract: str) -> DCEPlan:
        ...
        return _safe_make(
            DCEPlan,
            {
                ...
                "event_spine": data.get("event_spine", []),
        ...
        for idx, row in enumerate(rows):
            ...
            frame = _safe_make(
                StoryboardFrame,
                {
                    ...
                    "event_cue": row.get("event_cue", event),
                    "scene_cue": row.get("scene_cue", "The background and environment must visually support the event and emotion."),
                    ...
                    "supporting_cast": row.get("supporting_cast", []),
                    ...
                    "composition_rule": (
                        ...
                        "The scene must show both the event and the visual cause of the protagonist emotion."
                    ),
