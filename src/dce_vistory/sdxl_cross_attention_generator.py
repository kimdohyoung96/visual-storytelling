...
class SDXLButterflyCrossAttentionGenerator:
    def __init__(
        ...
        try:
            ...
            pass
        self.adapter_stack = ButterflyAdapterStack(character_tokens=character_tokens, world_tokens=world_tokens, emotion_tokens=emotion_tokens, event_tokens=event_tokens).to(device)
    ...
    def _encode_prompt(self, prompt: str, negative_prompt: str | None = None, do_cfg: bool = True):
        result = self.pipe.encode_prompt(prompt=prompt, device=self.device, num_images_per_prompt=1, do_classifier_free_guidance=do_cfg, negative_prompt=negative_prompt if negative_prompt is not None else "")
        if len(result) != 4:
    ...
    def generate_from_packet(self, packet: VisualControlPacket, frame_id: int, out_dir: Path, num_candidates: int = 1) -> List[CandidateImage]:
        ...
        strong_positive = packet.positive_prompt + """
...
"""
        strong_negative = packet.negative_prompt + ", monochrome, grayscale, black and white, pencil sketch, charcoal sketch, line art only, emotionless face, flat expression, stiff pose, flat lighting, colorless image, empty background, portrait only"
        prompt_embeds, negative_prompt_embeds, pooled_prompt_embeds, negative_pooled_prompt_embeds = self._encode_prompt(strong_positive, negative_prompt=strong_negative, do_cfg=True)
        ...
        augmented_prompt_embeds, adapter_tokens = self.adapter_stack(prompt_embeds, control_embeds=control_embeds, adapter_weights=packet.adapter_weights or {})
        if adapter_tokens.shape[1] > 0:
            negative_prompt_embeds = torch.cat([negative_prompt_embeds, torch.zeros_like(adapter_tokens)], dim=1)
        for cid in range(num_candidates):
            seed = self.seed + int(frame_id) * 1000 + cid
            generator = torch.Generator(device=self.device).manual_seed(seed) if self.device.startswith("cuda") else torch.Generator().manual_seed(seed)
            gs = self.guidance_scale + (0.5 if "intensity 5/5" in strong_positive.lower() or "emotion intensity: 5" in strong_positive.lower() else 0.0)
            image = self.pipe(prompt_embeds=augmented_prompt_embeds, negative_prompt_embeds=negative_prompt_embeds, pooled_prompt_embeds=pooled_prompt_embeds, negative_pooled_prompt_embeds=negative_pooled_prompt_embeds, width=self.width, height=self.height, num_inference_steps=self.num_inference_steps, guidance_scale=gs, generator=generator).images[0]
            path = out_dir / f"frame_{int(frame_id):03d}_cand_{cid:02d}.png"
            image.save(path)
            candidates.append(CandidateImage(
                frame_id=int(frame_id), candidate_id=cid, image_path=str(path), prompt=strong_positive,
                scores={"image_quality": 0.0, "identity_consistency": 0.0, "emotion_visibility": 0.0, "scene_alignment": 0.0, "event_alignment": 0.0, "colorfulness": 0.0, "overall": 0.0},
                notes={"seed": seed, "adapter_weights": packet.adapter_weights, "cross_attention_adapter_tokens": int(adapter_tokens.shape[1]), "negative_prompt": strong_negative, "full_color_enforced": True},
            ))
        return candidates
