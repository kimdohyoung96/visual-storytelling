...
def build_prompt_variant(contract: Dict[str, Any], mode: str = "event_first") -> str:
    ...
    if mode == "event_first":
        txt = f"full-color cinematic storybook illustration, same protagonist as reference image, {identity}. Scene: {world}. Action: {action}. Story beat: {story_sentence}. Visible evidence: {evidence}. Show {emotion} with {face} and {body}. Cause visible: {cause}. Include {env}. {light}, {color}, {comp}. Not a portrait only."
    elif mode == "evidence_first":
        txt = f"full-color narrative illustration, same subject as input reference image, {identity}. The scene must clearly show {evidence}. These visual clues must explain why the protagonist feels {emotion}. Story beat: {story_sentence}. Action visible: {action}. Background: {world}, {env}. {face}; {body}; {light}; {color}."
    elif mode == "emotion_first":
        txt = f"full-color expressive story illustration of the same protagonist from the reference image, {identity}. The protagonist feels {emotion}; show it strongly through {face} and {body}. The emotional cause must be visible: {cause}. Story beat: {story_sentence}. Event happening now: {action}. Show {evidence}. Use {world}, {light}, {color}, {comp}."
    else:
        txt = f"full-color continuous visual storytelling frame. Keep the same protagonist identity from reference image, {identity}, signature items {sig_items}. Advance the story from prior history: {contract['salient_history']}. Current story sentence: {story_sentence}. Current event: {action}. Must show {evidence}. World state: {world}, {env}. Emotion: {emotion}. {face}; {body}; {light}; {color}; {comp}."
    return limit_words(txt, 85)


def build_negative_variant(packet) -> str:
    base = clean_text(getattr(packet, "negative_prompt", ""))
    extra = "generic portrait, static portrait, repeated pose, missing action, missing event, missing evidence, missing hand interaction, cropped out important object, empty background, weak emotion, unclear cause, inconsistent protagonist, different subject from reference image, extra unrelated humans, human instead of animal, grayscale, monochrome, black and white, sketch, line-art only, blurry, low quality, text, watermark"
    return limit_words(base + ", " + extra, 85)
