...
class CrossAttentionButterflyDCEViStoryPipeline:
    def __init__(self, cfg: Dict[str, Any]):
        ...
        self.image_generator = SDXLButterflyCrossAttentionGenerator(
            ...
            width=int(icfg.get("width", 1024)),
            height=int(icfg.get("height", 1024)),
            num_inference_steps=int(icfg.get("num_inference_steps", 40)),
            guidance_scale=float(icfg.get("guidance_scale", 8.0)),
            seed=int(icfg.get("seed", 42)),
            adapter_ckpt=acfg.get("adapter_ckpt"),
            enable_cpu_offload=bool(icfg.get("enable_cpu_offload", False)),
            character_tokens=int(acfg.get("character_tokens", 8)),
            world_tokens=int(acfg.get("world_tokens", 8)),
            emotion_tokens=int(acfg.get("emotion_tokens", 8)),
            event_tokens=int(acfg.get("event_tokens", 8)),
        )
...
"""
        packet.negative_prompt += (
            ", grayscale, monochrome, black and white, pencil sketch, line art only, "
            "weak expression, emotionless face, flat pose, colorless image, empty background"
        )

        meta = packet.control_metadata or {}
        meta["emotion_text"] = meta.get("emotion_text", "") + (
            f"; emotion must be unmistakably visible through face, body, color, lighting and world state; "
            f"emotion evidence: {getattr(frame, 'emotion_evidence', [])}"
        )
        meta["world_text"] = meta.get("world_text", "") + (
            f"; full-color world state; weather: {getattr(frame, 'weather', '')}; "
            f"palette: {getattr(frame, 'color_palette', '')}; atmosphere: {getattr(frame, 'atmosphere', '')}"
        )
    ...
    def run(self, sample: Dict[str, Any], out_dir: Path) -> PipelineResult:
        ...
        save_json(_safe_asdict(seed), out_dir / "seed.json")
        (out_dir / "abstract.txt").write_text(abstract, encoding="utf-8")
        save_json(_safe_asdict(dce_plan), out_dir / "dce_plan.json")
        save_json(_safe_asdict(emotion_arc), out_dir / "emotion_arc.json")
        save_json([_safe_asdict(x) for x in storyboard], out_dir / "storyboard.json")
        ...
        save_json([_safe_asdict(x) for x in ending_candidates], out_dir / "ending_candidates.json")
    ...
    def _build_markdown(
        ...
        lines = []
        lines.append("# Final Emotion-Visible Visual Story\n")
        ...
        lines.append(f"- Ending Emotion: {dce_plan.target_ending_emotion}")
        ...
        for frame, image in zip(storyboard, images):
            ...
            lines.append(f"- Emotion Evidence: {getattr(frame, 'emotion_evidence', [])}")
            lines.append(f"- Shot: {getattr(frame, 'shot_type', '')}")
            lines.append(f"- Weather: {getattr(frame, 'weather', '')}")
            lines.append(f"- Palette: {getattr(frame, 'color_palette', '')}")
            lines.append(f"- Must Show: {getattr(frame, 'must_show', [])}")
            lines.append(f"- Scores: {image.scores}")
            lines.append(f"- Notes: {image.notes}\n")
