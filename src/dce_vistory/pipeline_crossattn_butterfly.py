        ...
        for pattern in patterns:
            for p in folder.glob(pattern):
                try:
                    ...
                    pass
    for name in ["contact_sheet.png", "candidate_manifest.json", "selected_images.json", "evaluation.json", "storyboard.json", "full_story.json", "full_story_partial.json", "storyboard_partial.json", "visual_control_packets.json"]:
        try:
            p = out_dir / name
            if p.exists():
                p.unlink()
...
def _make_contact_sheet_force(image_paths: List[str], out_path: Path, cols: int = 3, thumb_size: tuple[int, int] = (384, 384), title: str = "DCEE-CausalVerse V27 Visual Story") -> str:
...
class CrossAttentionButterflyDCEViStoryPipeline:
    """V27 caption-locked grounded DCEE pipeline.
    ...
    def run(self, sample: Dict[str, Any], out_dir: Path) -> PipelineResult:
        ...
        for idx in range(total_frames):
            ...
            try:
                setattr(frame, "previous_story_sentence", story_rows[-2].get("sentence", "") if len(story_rows) >= 2 else "")
            except Exception:
                pass
            ...
            if retry_enabled and (
                ...
                retry_candidates = self.image_generator.generate_from_packet(packet=strong_packet, frame_id=frame_id, out_dir=target_dir, num_candidates=max(3, candidate_count + 1))
        ...
        _write_json(out_dir / "output_manifest.json", {
            ...
            "generation_policy_V27": str(out_dir / "generation_policy_V27.json"),
