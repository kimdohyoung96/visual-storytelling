...
def _safe_asdict(obj: Any):
    """Convert dataclass/object/dict/list into JSON-friendly Python values."""
    if is_dataclass(obj):
        d = asdict(obj)
...
def _write_json(path: Path, obj: Any):
    """Always write JSON with UTF-8. This is independent of utils.save_json behavior."""
...
def _make_contact_sheet_force(
    image_paths: List[str],
    out_path: Path,
    cols: int = 3,
    thumb_size: tuple[int, int] = (384, 384),
    title: str = "DCEE-CausalVerse Visual Story",
) -> str:
    """
    Robust contact sheet generator.

    This function is intentionally implemented inside the pipeline so that
    contact_sheet.png is created regardless of utils.make_contact_sheet.
    """
...
class CrossAttentionButterflyDCEViStoryPipeline:
    """
    Complete stable DCEE-CausalVerse pipeline.

    The pipeline follows the paper direction:
    Desire -> Conflict -> multiple Event Chains -> selected Ending Emotion path
    + Causal Sink Memory
    + Anchor Bank
    + Decoder-Encoder-Decoder Butterfly Visual Controller
    + Character/World/Emotion/Event/Evidence cross-attention controls
    + event/evidence-aware reranking and retry
    + guaranteed contact_sheet.png / evaluation.json / final_story.md outputs.
    """
    ...
    def _strengthen_packet(self, packet, frame):
        """Strengthen event/evidence/emotion controls for retry generation."""
        packet.positive_prompt += (
            ...
            f"\n- The visual evidence must appear: {getattr(frame, 'emotion_evidence', [])}"
            f"\n- The emotion must be readable: {getattr(frame, 'emotion', '')}"
            "\n- Use rich full color. Do not create grayscale or monochrome output."
        )
        packet.negative_prompt += (
            "; missing event, missing evidence, weak emotion, emotionless face, "
            "missing visual cause, grayscale, monochrome, empty background"
        )
        ...
        packet.adapter_weights["event_adapter"] = min(0.48, packet.adapter_weights.get("event_adapter", 0.18) + 0.10)
        packet.adapter_weights["evidence_adapter"] = min(0.52, packet.adapter_weights.get("evidence_adapter", 0.20) + 0.14)
        packet.adapter_weights["emotion_adapter"] = min(0.50, packet.adapter_weights.get("emotion_adapter", 0.28) + 0.08)
        packet.adapter_weights["world_adapter"] = min(0.35, packet.adapter_weights.get("world_adapter", 0.18) + 0.05)
    ...
    def _save_core_plan_outputs(self, out_dir: Path, seed, abstract, full_story, dce_plan, emotion_arc, storyboard):
        ...
        if candidate_plans is None:
            candidate_plans = {
                "note": "No explicit candidate plan list was attached by planner.py.",
                "selected_event_chain": getattr(dce_plan, "event_chain", getattr(dce_plan, "event_spine", [])),
            }
    ...
    def run(self, sample: Dict[str, Any], out_dir: Path) -> PipelineResult:
        ...
        # 1. Input understanding + DCEE planning
        ...
        for idx, frame in enumerate(storyboard):
            ...
            try:
                selected_memory = memory.select(
                    frame,
                    dce_plan,
                    emotion_arc,
                    strategy=pipe_cfg.get("memory_strategy", "adaptive_causal"),
                )
            except Exception as e:
            ...
            candidates = self.image_generator.generate_from_packet(
                packet=packet,
                frame_id=frame_id,
                out_dir=target_dir,
            ...
            ranked = (
                self.evaluator.rerank_ending_candidates(frame, dce_plan, candidates)
                if is_last
                else self.evaluator.rank_frame_candidates(frame, dce_plan, candidates, is_ending=False)
            )
            ...
            # Retry if DCEE-specific grounding/emotion/color scores are weak.
            if retry_enabled and (
                ...
                retry_candidates = self.image_generator.generate_from_packet(
                    packet=strong_packet,
                    frame_id=frame_id,
                    out_dir=target_dir,
                    num_candidates=max(2, candidate_count // 2),
                )

                retry_ranked = (
                    self.evaluator.rerank_ending_candidates(frame, dce_plan, retry_candidates)
                    if is_last
                    else self.evaluator.rank_frame_candidates(frame, dce_plan, retry_candidates, is_ending=False)
                )
            ...
            packet_log.append(
                {
                    "frame_id": frame_id,
                    "visual_control_packet": _safe_asdict(packet),
                    "anchors": anchors,
                    "retried": retried,
                }
            )
            memory_log.append(
                {
                    "frame_id": frame_id,
                    "memory": selected_memory,
                    "selected_image": _safe_asdict(best),
                    "all_candidates": [_safe_asdict(c) for c in ranked],
                }
            )
            candidate_manifest.append(
                {
                    "frame_id": frame_id,
                    "is_ending": is_last,
                    "selected": _safe_asdict(best),
                    "candidate_paths": [getattr(c, "image_path", "") for c in ranked],
                    "num_ranked_candidates": len(ranked),
                }
            )

            # Save incremental outputs after each frame so partial long runs are inspectable.
        ...
        # 4. Contact sheet must be generated before evaluation.
        contact_sheet_path = None
        contact_sheet_error = None
        try:
            contact_sheet_path = _make_contact_sheet_force(
                [getattr(x, "image_path", "") for x in selected_images],
                out_dir / "contact_sheet.png",
            )
        except Exception as e:
            contact_sheet_error = f"{type(e).__name__}: {e}"
            run_errors.append({"stage": "contact_sheet", "error": contact_sheet_error, "traceback": traceback.format_exc()})

        # 5. Evaluation. Always save evaluation.json.
        try:
            questions = self.evaluator.generate_questions(dce_plan, emotion_arc, storyboard)
        except Exception as e:
            questions = {"error": "generate_questions_failed", "error_type": type(e).__name__, "message": str(e)}
            run_errors.append({"stage": "generate_questions", "error": str(e), "traceback": traceback.format_exc()})

        _write_json(out_dir / "eval_questions.json", questions)

        try:
            evaluation = self.evaluator.evaluate_sequence(
                dce_plan,
                emotion_arc,
                storyboard,
                selected_images,
            ...
            evaluation = {
                "error": "evaluate_sequence_failed",
                "error_type": type(e).__name__,
                "message": str(e),
                "num_selected_images": len(selected_images),
                "selected_image_paths": [getattr(x, "image_path", "") for x in selected_images],
            }
        ...
        # 6. Markdown result. Always save final_story.md.
        try:
            ...
            final_story_md = (
                ...
                f"- Message: {str(e)}\n"
                "- Check selected_images.json, candidate_manifest.json, evaluation.json, and contact_sheet.png.\n"
            )
    ...
    def _build_markdown(abstract, full_story, dce_plan, emotion_arc, storyboard, images, ending_candidates, evaluation):
        lines = [
            ...
            *([f"  - [{row.get('frame_id', i+1)}] {row.get('sentence', '')}" for i, row in enumerate((full_story or {}).get('sentences', []))] if isinstance(full_story, dict) else []),
            ...
            f"- Planning Structure: {getattr(dce_plan, 'planning_structure', 'DCEE: Desire-Conflict-Event-Ending Emotion')}",
        ...
        for frame, image in zip(storyboard, images):
            lines += [
                ...
                f"- Emotion Intensity: {getattr(frame, 'emotion_intensity', '')}/5",
                f"- Conflict Level: {getattr(frame, 'conflict_level', '')}/5",
                f"- Evidence: {getattr(frame, 'emotion_evidence', [])}",
        ...
        for cand in ending_candidates:
            lines += [
                f"### Candidate {getattr(cand, 'candidate_id', '')}",
                f"![Candidate {getattr(cand, 'candidate_id', '')}]({getattr(cand, 'image_path', '')})",
                f"- Scores: {getattr(cand, 'scores', {})}",
                "",
            ]
